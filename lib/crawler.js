...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
	...
	var doRequest = function(err) {
		...
		cacache.get('/tmp/tmp_cache', options.uri).then(
			function (data) {
				console.log('resolved 缓存');
				const response = JSON.parse(data.data, (key, value) => {
